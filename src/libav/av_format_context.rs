...
use super::{AVError, AVStream, AVPacket};
...
impl AVFormatContext {
    pub fn new(input_path: &str) -> Result<Self, AVError> {
        ...
        let open_result = unsafe { ff::avformat_open_input(
            &mut avctx,
    ...
    pub fn get_streams(&self) -> Result<Vec<AVStream>, AVError> {
        ...
        let av_streams = unsafe { std::slice::from_raw_parts((*self.ctx).streams, (*self.ctx).nb_streams as usize) };
        return Ok(av_streams
            ...
            .filter_map(|(i, &stream)| {
                ...
                let codec: *mut ff::AVCodec = unsafe { ff::avcodec_find_decoder((*codec_params).codec_id) };
                if codec.is_null() {
                    return None;
                }
    ...
    pub fn read_frame(&self, packet: &mut AVPacket) -> Option<()> {
