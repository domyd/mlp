...
pub mod log {
    ...
    pub fn configure_rust_log(level: LevelFilter) {
        let ffmpeg_log_level = match level {
            LevelFilter::Off | LevelFilter::Trace | LevelFilter::Debug => 40,
            LevelFilter::Info => 32,
            LevelFilter::Warn => 24,
            LevelFilter::Error => 16,
        };

        unsafe {
            ff::av_log_set_level(ffmpeg_log_level);
    ...
    extern "C" fn ffmpeg_log_adapter(
        ...
        if let Some(log_level) = match level {
            ...
            if let Some(message_without_newline) = match message.chars().last() {
                ...
                log!(target: "ffmpeg", log_level, "{}", message_without_newline);
            }
        }
    }
}
